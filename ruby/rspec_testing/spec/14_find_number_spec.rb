...
describe FindNumber do
  ...
  describe '#game_over?' do
    ...
    context 'when guess and random_number are equal' do
      ...
      subject(:game_win) { described_class.new(3, 8, random_number, 5) }
      it 'is game over' do
    ...
    context 'when guess and random_number are not equal' do
      # let(:random_number) { double('random_number', value: 5) }
      subject(:game_lose) { described_class.new(4, 7, random_number, 6) }
      it 'is not game over' do
        allow(random_number).to receive(:value).and_return(5)
        expect(game_lose).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:new_guess_game) { described_class.new(5, 8, number_range, 7) }

      before do
        new_guess_game.update_range
      end

      it 'updates min to the same value as max' do
        min = new_guess_game.min
        expect(min).to eq(8)
      end

      it 'does not update max' do
        max = new_guess_game.max
        expect(max).to eq(8)
      end
    end
  end
